...
import type { IProduct, IProductFilters, TCategory } from './types'
...
export const useProductStore = defineStore('productStore', {
  ...
  getters: {
    filteredProducts: ({ filters: { category, search }, products }) =>
      products.filter((p) => {
        const isCategoryMatch = category === 'all' || p.category === category

        return isCategoryMatch && p.name.toLowerCase().includes(search.toLowerCase())
      }),
  },
  actions: {
